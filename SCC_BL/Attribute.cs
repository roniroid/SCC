...
namespace SCC_BL
{
    public class Attribute : IDisposable
    {
        ...
        public int ConsecutiveValue { get; set; }
        ...
        //For SelectByIDHierarchy (RESULT)
        public Attribute(int id, int formID, string name, string description, int errorTypeID, int? parentAttributeID, int? maxScore, bool topDownScore, bool hasForcedComment, bool isKnown, bool isControllable, bool isScorable, int order, int basicInfoID, int consecutiveValue)
        {
            this.ID = id;
            this.FormID = formID;
            this.Name = name;
        ...
        public static Attribute AttributeFromExcelCell(DocumentFormat.OpenXml.Spreadsheet.Cell[] cells, SCC_BL.DBValues.Catalog.ATTRIBUTE_ERROR_TYPE attributeErrorType, int formID, int attributeNameIndex, int attributeDataStartIndex, int actualGhostID, int parentAttributeGhostID, int order, int creationUserID)
        {
            Attribute @object = new Attribute();
            @object.ErrorTypeID = (int)attributeErrorType;
            ...
            using (SCC_BL.Tools.ExcelParser excelParser = new Tools.ExcelParser())
            {
                ...
                int maxScore =
                    ...
                    ? Convert.ToInt32(excelParser.GetCellValue(cells[attributeDataStartIndex + (int)SCC_BL.Settings.AppValues.ExcelTasks.Form.UploadForm.AttributeFields.MAX_SCORE]).ToString().Trim())
                    : 0;
                @object.MaxScore = maxScore;

                string topDownScoreCellValue = excelParser.GetCellValue(cells[attributeDataStartIndex + (int)SCC_BL.Settings.AppValues.ExcelTasks.Form.UploadForm.AttributeFields.TOP_DOWN_SCORE]).ToString().Trim().ToLower();
                topDownScoreCellValue = @object.TranslateBoolean(topDownScoreCellValue);

                bool topDownScore =
                    !string.IsNullOrEmpty(topDownScoreCellValue)
                        ? Convert.ToBoolean(topDownScoreCellValue)
                        : false;
                @object.TopDownScore = topDownScore;

                string forceCommentCellValue = excelParser.GetCellValue(cells[attributeDataStartIndex + (int)SCC_BL.Settings.AppValues.ExcelTasks.Form.UploadForm.AttributeFields.FORCE_COMMENT]).ToString().Trim().ToLower();
                forceCommentCellValue = @object.TranslateBoolean(forceCommentCellValue);

                bool forceComment =
                    !string.IsNullOrEmpty(forceCommentCellValue)
                    ? Convert.ToBoolean(forceCommentCellValue)
                    : false;
                @object.HasForcedComment = forceComment;

                string knownCellValue = excelParser.GetCellValue(cells[attributeDataStartIndex + (int)SCC_BL.Settings.AppValues.ExcelTasks.Form.UploadForm.AttributeFields.KNOWN]).ToString().Trim().ToLower();
                knownCellValue = @object.TranslateBoolean(knownCellValue);

                bool known =
                    !string.IsNullOrEmpty(knownCellValue)
                    ? Convert.ToBoolean(knownCellValue)
                    : false;
                @object.IsKnown = known;

                string controllableCellValue = excelParser.GetCellValue(cells[attributeDataStartIndex + (int)SCC_BL.Settings.AppValues.ExcelTasks.Form.UploadForm.AttributeFields.CONTROLLABLE]).ToString().Trim().ToLower();
                controllableCellValue = @object.TranslateBoolean(controllableCellValue);

                bool controllable =
                    !string.IsNullOrEmpty(controllableCellValue)
                    ? Convert.ToBoolean(controllableCellValue)
                    : false;
                @object.IsControllable = controllable;

                string scorableCellValue = excelParser.GetCellValue(cells[attributeDataStartIndex + (int)SCC_BL.Settings.AppValues.ExcelTasks.Form.UploadForm.AttributeFields.SCORABLE]).ToString().Trim().ToLower();
                scorableCellValue = @object.TranslateBoolean(scorableCellValue);

                bool scorable =
                    !string.IsNullOrEmpty(scorableCellValue)
                    ? Convert.ToBoolean(scorableCellValue)
                    : false;
                @object.IsScorable = scorable;
                ...
                if (!string.IsNullOrEmpty(defineAnswerType))
                {
                    for (int i = 0; i < defineAnswerType.Split('/').Length; i++)
                    {
                        ...
                        @object.ValueList.Add(attributeValueCatalog);
            ...
            @object.FormID = formID;
            @object.AttributeGhostID = actualGhostID;
            @object.ParentAttributeGhostID = parentAttributeGhostID;
            @object.Order = order;

            @object.BasicInfo = new BasicInfo(creationUserID, (int)SCC_BL.DBValues.Catalog.STATUS_ATTRIBUTE.CREATED);
        ...
        public List<Attribute> SelectHierarchyByFormID(bool simplifiedInfo = false)
        {
            ...
            using (SCC_DATA.Repositories.Attribute repoAttribute = new SCC_DATA.Repositories.Attribute())
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    /*if (!simplifiedInfo)
                    {
                        attribute.ChildrenList = GetChildren(attribute.ID, attributeList.ToArray());
                    }*/
                ...
                if (!simplifiedInfo)
                {
                    ...
                    {
                        //currentAttribute.ChildrenList = GetChildren(currentAttribute.ID, attributeList.ToArray());
                        currentAttribute.ChildrenList = GetChildren(currentAttribute.ID, attributeList);
                    }
                }
            }

            return attributeList
                .OrderBy(e => e.ConsecutiveValue)
                .ToList();
        }

        public List<Attribute> SelectHierarchyByFormID_OLD_20240502150400(bool simplifiedInfo = false)
        {
            List<Attribute> attributeList = new List<Attribute>();
            ...
            using (SCC_DATA.Repositories.Attribute repoAttribute = new SCC_DATA.Repositories.Attribute())
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        //attribute.ChildrenList = GetChildren(attribute.ID, attributeList.ToArray());
                        attribute.ChildrenList = GetChildren(attribute.ID, attributeList);
                    }
        ...
        public List<Attribute> SelectHierarchyByFormIDSync(bool simplifiedInfo = false)
        {
            List<Attribute> attributeList = new List<Attribute>();
            ...
            using (SCC_DATA.Repositories.Attribute repoAttribute = new SCC_DATA.Repositories.Attribute())
            {
				...
				Parallel.ForEach<System.Data.DataRow>(dt.Rows.Cast<DataRow>().ToList(), async (dr) => {
                    ...
                    Attribute attribute = new Attribute(
                        ...
                        Convert.ToInt32(dr[SCC_DATA.Queries.Attribute.StoredProcedures.SelectHierarchyByFormID.ResultFields.ORDER]),
                        Convert.ToInt32(dr[SCC_DATA.Queries.Attribute.StoredProcedures.SelectHierarchyByFormID.ResultFields.BASICINFOID]),
                        Convert.ToInt32(dr[SCC_DATA.Queries.Attribute.StoredProcedures.SelectHierarchyByFormID.ResultFields.CONSECUTIVE_VALUE])
                    );
                    ...
                    // Add the attribute to the list inside the lock to avoid race conditions
                    lock (attributeList)
                    {
                        attributeList.Add(attribute);
                    }

                    tasks.Add(Task.CompletedTask); // Add a completed task for each iteration
                });

                // Wait for all tasks within Parallel.ForEach to complete
                Task.WhenAll(tasks);*/
            ...
            /*Attribute[] newAttributeArray = attributeList.ToArray();

            if (!simplifiedInfo)
            {
                for (int i = 0; i < newAttributeArray.Length; i++)
                {
                    newAttributeArray[i].ChildrenList = GetChildren(newAttributeArray[i].ID, newAttributeArray);
                }
            }*/
        ...
        /*private async Task<List<Attribute>> GetChildren(int attributeID, Attribute[] attributeList) 
		{
			List<Attribute> result = new List<Attribute>();
            ...
            foreach (SCC_BL.CustomTools.FormUploadInfo.ErrorTypeInfo errorTypeInfo in formUploadInfo.ErrorTypeList)
            {
                ...
                foreach (SCC_BL.CustomTools.FormUploadInfo.ErrorTypeInfo.AttributeListInfo attributeInfo in errorTypeInfo.AttributeList)
                {
                    ...
                    using (SCC_BL.Tools.ExcelParser excelParser = new Tools.ExcelParser())
                    {
                        ...
                        Attribute attribute = Attribute.AttributeFromExcelCell(
                            auxCurrentCells,
